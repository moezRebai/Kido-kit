...
The CLI is built and working (`C:\Solutions\Kido`, npm package `kido`, symlink-installed via `npm link`, verified against a real brownfield repo at `D:\HedgeR`, and against a real spotPricer Jira instance). 58 automated tests passing. The design below reflects the **current** state — corrections made after real usage are folded directly into the relevant sections rather than kept as a separate patch list, except for the two items below that are still genuinely open.

Two corrections from real Jira usage, already folded into the CLI subcommands table above: (1) Jira Cloud removed the `GET /rest/api/3/search` endpoint kido's Story-listing relied on (`410 Gone`) — migrated to `/rest/api/3/search/jql` with cursor-based pagination. (2) Epics/feature-Stories were dumping raw markdown source into a single plain-text Jira description field (literal `##`/`-`/code-fence characters, no real Jira formatting) — switched to real ADF encoding for all descriptions, plus attach-and-summarize for Epics/feature-Stories specifically (short auto-extracted summary in the description, full `functional-spec.md`/`design.md` as real attachments, downloaded byte-exact on pull).
...
| `kido jira pull <key> [--as <change-name>]` | The reverse of `kido jira sync` — given any Jira key, materializes the whole local change into `kido/changes/<name>/`. What comes back depends on the key: a Kido-created Epic (or one of its task Stories) resolves to `functional-spec.md` + `design.md` (downloaded byte-exact from the Epic's attachments, not parsed from the description — logs a note and skips instead of failing if an attachment is missing, e.g. an Epic created via `--epic` that was never synced with a spec on disk) + `tasks.md` (reconstructed from every child Story's description); a **small-feature Story** (self-contained — detected via a stable `## Functional Spec` description prefix, before any parent walk) resolves the same way, just `functional-spec.md` (+ `design.md` if attached), no `tasks.md`; a Bug resolves to `bug.md` alone. Only reads the current attach-based format — a ticket created before this format shipped needs one more `kido jira sync` to migrate before it can be pulled. This is how Dev picks up work BA pushed to Jira but never had repo push access to commit. |
...
1. **Unit tests** (`npm test`, 58 passing) for the CLI's file/data operations: `kido init` scaffolding, `status`/`validate` logic, `archive`'s move operation, the markdown↔ADF converter, and the Jira client (against a fake local Jira server, exercising create-or-update idempotency for Epic/Story/Bug, the `--epic` override, `/search/jql` pagination, and attachment upload/delete/download for the Epic/feature-Story attach-and-summarize path).
...
flowchart TD
    ...
    JIRA_E[("Jira Epic<br/>(summary description;<br/>functional-spec.md + design.md attached — no git needed)")]
