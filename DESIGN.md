...
| `kido jira pull <key> [--as <change-name>]` | The reverse of `kido jira sync` — given any Jira key (Epic, Story, or Bug), materializes the whole local change into `kido/changes/<name>/`: a Story resolves its parent Epic first, so the result always includes `functional-spec.md` + `design.md` + `tasks.md` (reconstructed from every child Story), or just `bug.md` for a Bug. This is how Dev picks up work BA pushed to Jira but never had repo push access to commit. |
...
`functional-spec.md` → Epic only. `tasks.md` tasks → Stories directly (no Sub-task level). `bug.md` → Bug ticket, same tracker, distinct type. `--epic <KEY>` on `kido jira sync` overrides the auto-Epic when one doesn't exist or isn't the right one. Every sync point explicitly asks first — never silent.
...
flowchart TD
    ...
    SPECIFY["/kido:specify — BA, one session<br/>functional-spec.md, then design.md<br/>(Dev optional — BA drafts design.md<br/>solo if Dev isn't present)<br/>brainstorming-style grill +<br/>boundary/failure/concurrency enumeration,<br/>reads FD + TD"]
...
*(The bug path — `/kido:specify`'s first fork — isn't drawn above since this diagram traces the feature path only; see the `/kido:specify` row in the command table and Validated use case #4 below for the bug lane.)*
...
| **BA/Dev handoff & Jira** | **Post-implementation revision**: BA has no git commit/push access, so Jira — not git — is the transport between BA and Dev. `/kido:specify` is now one merged session (not two dispatched passes) producing `functional-spec.md` + `design.md` together, whether Dev is present or not; both push into the same Epic's description as two labeled sections, since Jira's hierarchy has no separate tier for design.md. `tasks.md` tasks still sync as Stories directly, no sub-task level (#39). `--epic` override for Dev-only/pre-existing Epics. `kido jira pull <key>` (new) is the reverse — Dev materializes the whole local change from any Epic/Story/Bug key, since BA's output never reaches them via git. Every sync point (either direction) always asks first, never silent (#28). |
