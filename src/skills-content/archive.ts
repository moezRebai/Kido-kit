...
2. **Ask**: "Want me to update \`kido/docs/\` with anything from this change?" If yes, judge the change's impact first:
   - **Trivial** (small addition or simple bug fix — no new capability, no structural/API change worth a reader knowing about): skip \`/kido:document\` entirely. Directly edit the relevant section(s) of \`kido/docs/{project}-functional-docs.md\`/\`technical-docs.md\` in place — a line or two, grounded in the diff and artifacts already in context. No discovery phases, no other skill invocation, no reads outside this change's own files.
   - **Otherwise** (new capability, structural change, new integration surface, etc.): re-invoke \`/kido:document\` in its scoped, incremental mode — seeded with this change's own artifacts (\`functional-spec.md\`/\`design.md\`/\`tasks.md\`/\`bug.md\` + the actual diff) plus the affected code area only. No repo-wide rescan, and no reads outside that scope.
